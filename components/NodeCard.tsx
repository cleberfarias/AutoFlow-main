...
const NodeCard: React.FC<NodeCardProps> = ({ step, isActive, isPanningMode, onEdit, onDelete, onMove }) => {
  ...
  return (
    <div
      className={`NodeCard absolute w-[360px] bg-white rounded-[28px] border-[1.5px] transition-all p-0 select-none flex flex-col ${theme.border} ${theme.shadow} ${isDragging ? 'z-[100] scale-[1.03] cursor-grabbing' : 'z-10'}`}
      style={{ left: pos.x, top: pos.y }}
      onMouseDown={handleMouseDown}
    >
      <div className={`node-handle handle-in ${theme.border}`} />
      <div className={`node-handle handle-out ${theme.border}`} />

      <div className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className={`w-11 h-11 ${theme.accent} rounded-2xl flex items-center justify-center shadow-lg`}>
              <Icon size={18} className="text-white" fill={step.type === StepType.TRIGGER ? 'currentColor' : 'none'} />
            </div>
            <div>
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">{step.type}</span>
              <h3 className="text-slate-900 font-extrabold text-base leading-tight">{step.title}</h3>
            </div>
          </div>
          <div className="flex gap-1">
            <button onClick={() => onEdit(step.id)} className="p-2.5 rounded-xl hover:bg-slate-50 text-slate-400 transition-colors"><Settings size={18} /></button>
            <button onClick={() => onDelete(step.id)} className="p-2.5 rounded-xl hover:bg-rose-50 text-rose-300 transition-colors"><Trash2 size={18} /></button>
          </div>
        </div>
        <p className="text-slate-500 text-[13px] leading-relaxed mb-4 line-clamp-2">{step.description}</p>
      ...
      {step.params?.url && (
        <div className="bg-[#0f172a] p-5 rounded-b-[27px] mx-[1px] mb-[1px]">
